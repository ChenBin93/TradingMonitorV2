...
| c27 | ER 方向效率轴（Kaufman CH17 灵感，书 P1） | c12/c14/c15/c17 + 书"噪声≠波动" | ER_n=\|C_t−C_{t−n}\|/Σ\|C_i−C_{i−1}\|（n=10，Kaufman 默认），rolling 80/20 分位离散化（causal.rolling_percentile，禁全样本分位）。H1（正交性）：ER 高分位（≥80th）触碰在低/中/高波动状态（z120 三分位）下均有分布（每格 ≥10%）——ER 是独立于波动的新轴；H2（释放调节）：高 ER 触碰 vs 低 ER（≤20th）触碰的 24h 波动释放（E1，c15 口径）净差 ≥ 3pp，GBM 同管线净差 < 1pp（"噪声≠波动"的可交易化）；H3（折返调节）：趋势态+高 ER 触碰的端点折返（D1，c17 口径）比低 ER 少 ≥ 2pp，分年 3/3 同号。描述层；GBM 30 种子；1h 为主 4h 交叉。**若 H1-H3 确认 → c28（ER 自适应止损：Knapp 规则——6ATR 初始止损，ER>0.3 收紧 0.1ATR、ER>0.6 收紧 0.2ATR，重攻 trail 死穴）** |
