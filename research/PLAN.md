...
| c32 | ER 频率梯度的传统市场对照（U0-1 补验：书断言在书的市场成立吗？用户要求） | c30 同管线 + c31 gap 匹配 null | 同 c30 管线：ER_10 中位数 1h vs 1d。加密 BTC/ETH 1h+日线（c30 口径）；传统对照 SPY/CL=F/GC=F/EURUSD=X/^TNX（control.db，1h 原始序列 + 1d；4h 无源跳过；会话缺口混入 ER 窗口=设计偏离，docstring 注明）。H1：传统市场 ER_10 中位数 1h < 1d（5 标的全部，书"频率越低噪声越低"在其语境成立）；H2：传统日线−1h 差（5 标的均值）− 加密日线−1h 差（BTC/ETH 均值） ≥ 2pp（传统有梯度、加密没有 → 加密特殊性）；H3：GBM 同管线无梯度（30 种子，**gap 匹配 null**：按各对照标的会话日历掩码 GBM bar，c31 模式）。学习级：无 BY_YEAR、MIN_N=100、描述层、[学习级] |
