...
| c38 | 收益自相关谱 + DW 统计（U0-6：书 CH2 p.43-58 统计推断） | c31（反持久）/c12（波动长记忆）/c18（漂移） | BTC/ETH 1h/4h/日线 + SPY/GC=F/EURUSD 日线对照。收益自相关谱 ρ(lag 1..20) + DW≈2(1−ρ₁) + 平方收益自相关。H1：加密 1h 收益 ρ(1) < 0 且超 GBM 95% 区间（反持久）；H2：存在滞后 k 使 \|ρ(k)\| 回到 GBM 区间（反持久消失边界——"近零持久"边界）；H3：平方收益自相关显著为正且慢衰减（c12 长记忆的滞后表达）；H4：传统市场日线 ρ(1) 符号报告（书 CH2 暗示趋势市场应正自相关——对照，不设门槛）。学习级：30 种子、无 BY_YEAR、MIN_N=100、描述层、[学习级] |
| c39 | 分布矩峰度 × 市场状态（U0-5：书 CH2 p.37-43"负峰度=趋势/扁平、正峰度=横盘/聚集"） | c27（ER 分位状态）/c13（偏度）/c31 | BTC/ETH 1h：收益超额峰度按市场状态分组——ER rolling 分位（c27 口径）：高 ER 分位=趋势态、低 ER 分位=横盘态（causal.rolling_percentile，因果）。H1：趋势态收益峰度 < 横盘态峰度（书的方向）；H2：真实峰度与 GBM 同管线对照（真实偏离 null 的方向与幅度；GBM 峰度≈0 超额）。学习级：30 种子、无 BY_YEAR、MIN_N=100、描述层、[学习级] |
