...
铁律两档都不松：GBM 零假设对照、无未来函数、docstring 预注册、GATE 自检、check_study。

- 学习级结论标注 `[学习级]`，不得作交易依据；升级研究级须补 20 标的 × 30 种子 × BY_YEAR 重跑。
...
| c31 | 运行肥尾 vs GBM + 传统市场对照（学习单元 U0-2：书 CH1/CH8"运行序列肥尾=趋势系统利润唯一来源"） | U0-2 框架 + c12 | 加密 BTC/ETH 1h/4h 同向连续 run 长分布 vs GBM；传统对照 SPY/GC=F/EURUSD=X 1h 同口径同管线。H1：加密长 run 频率 > GBM（肥尾存在）；H2：传统市场长 run 频率 > GBM（书在其语境成立）；H3：加密 vs 传统长 run 频率差 \|Δ\| ≥ 2pp（加密特殊性）。run 断裂规则：相邻 bar 间隔 > 2×标准 bar 间隔则断 run（对照组有隔夜/周末缺口）。**学习级**：GBM 10 种子、无 BY_YEAR、MIN_N=100、描述层、结论标 [学习级] |
