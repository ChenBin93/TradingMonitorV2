...
| c33 | 日线运行肥尾忠实复现（Q2 书原口径：日线+对称硬币+双基准） | c31（run 状态机）+ c18（漂移混淆） | run=纯符号 sign(close_t−close_{t−1})，0 变化计延续（声明）。传统 5 标的日线 **max 历史**（SPY 1993-/CL=F 2000-/GC=F 2000-/EURUSD 2003-/^TNX 1962-，control.db 已更新）；加密 BTC/ETH 日线 3y。基准 1（忠实版）：对称硬币 iid p=0.5、30 种子、n=各自 bar 数；基准 2（修正版）：漂移=样本均值、σ=样本 sd 的 GBM、30 种子（c18：书用对称硬币会把漂移误判为肥尾）。统计量 N(run≥k)，k∈{6,8,10}，主判据 k=6（书锚点"run of 6"），另报 P(run≥12)。H1：传统市场 N(run≥6) 超出硬币 95% 区间（书在其语境成立）；H2：加密 N(run≥6) 超出硬币 95% 区间；H3：漂移 GBM 基准下真实仍超出（肥尾超出漂移可解释部分）。学习级：30 种子、无 BY_YEAR、MIN_N=100、描述层、[学习级] |
...
| c35 | ER 固定日历窗校准（Q1 书原口径：20 日历日窗口） | c30/c32（口径修正） | BTC/ETH 1h + 4h：ER 窗口固定日历 20 日（1h=480 bar、4h=120 bar），逐 bar 滚动取全样本均值。校准检查：1h 重采样成 4h 对齐序列验证 close-based ER 不变性（数学预期 Σ\|单步变化\| 跨频率几乎相等）。H1：avgER(1h) ≈ avgER(4h)（\|差\| < 2pp——书断言在固定日历窗口径下退化为不变性，诚实报告）；H2：GBM 同管线同结果。学习级：30 种子、无 BY_YEAR、MIN_N=100、描述层、[学习级] |
