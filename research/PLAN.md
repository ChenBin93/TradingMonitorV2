...
| c52 | E1 时间剖面（用户质疑：±12bar 窗口稀释峰值——把波动释放画成曲线） | c51（带触碰管线）/c15（E1 口径） | BTC/ETH 1h+4h。事件=收盘出 MA20±2σ 带（c51 管线）；E1(h)=mean(ATR[t+1..t+h])/mean(ATR[t−h..t−1])−1，h∈{1,3,6,12}；GBM 30 种子同管线逐 horizon null。H1：释放前载——E1(1) 净差（真实−GBM）> E1(12) 净差（峰值在头根附近）；H2：全部 horizon 净差 > 0 且超 2σ（释放全时段为正）；H3：1h 与 4h 标准化剖面形状一致（净差曲线单调性同向）。学习级：30 种子、无 BY_YEAR、MIN_N=100、描述层、[学习级] |
