...
def _targets(close_i, atr_i, t_mult, direction):
    dist = atr_i * t_mult
    if direction == "long":
...
def evaluate_forward(close, high, low, atr, entries, direction="long",
    ...
    for i in np.flatnonzero(entries):
        if i + 1 >= n or not np.isfinite(close[i]) or not np.isfinite(atr[i]) or atr[i] <= 0:
            continue
        tp, sl = _targets(close[i], atr[i], t_mult, direction)
...
def evaluate_forward_vbt(close, high, low, atr, entries, direction="long",
                         t_mult=T, w=W, open_px=None, chunk=64) -> tuple[Outcome, list[TradeRec]]:
    ...
    n = len(close)
    idx = pd.RangeIndex(n)
    pct = np.divide(atr * t_mult, close, out=np.full(n, np.nan), where=close != 0)
    pct_cols = pd.DataFrame(np.tile(pct[:, None], (1, chunk)), index=idx)

    entry_idxs = [i for i in np.flatnonzero(entries)
                  if i + 1 < n
                  and np.isfinite(close[i]) and np.isfinite(atr[i]) and atr[i] > 0]
    out = Outcome()
    recs: list[TradeRec] = []
    for ci in range(0, len(entry_idxs), chunk):
        ...
        pf = vbt.Portfolio.from_signals(
            ...
            size=1.0, sl_stop=pct_cols, tp_stop=pct_cols,
        )
        ...
        for r in pf.trades.records.to_dict("records"):
            ...
            tp, sl = _targets(close[e_i], atr[e_i], t_mult, direction)
...
def report_wr(outcome: Outcome, label: str = "") -> str:
    ...
    head = f"{label}: n={outcome.n_eval} (win {outcome.n_win} / loss {outcome.n_loss} / 过期 {outcome.n_expired} / 跳过 {outcome.n_skip})"
