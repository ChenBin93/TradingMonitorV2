#!/usr/bin/env python3
"""416天: 三层趋势 × 方向对齐RS 增量验证

正确用法: 做多时偏好 RS 强(正), 做空时偏好 RS 弱(负)
aligned_rs = rs if 做多 else -rs  →  aligned_rs > 0 表示 RS 与方向一致
"""
import sys
import numpy as np
import pandas as pd
...
# ── 主循环: 三层组合 × 方向对齐RS × 顺势方向 1:1 ──
# aligned_rs = rs × 方向符号 (做多: +rs; 做空: -rs)
...
for sym in syms:
    ...
    for i in range(100, n - 25):
        ...
        if not win and not loss:
            continue
        # 方向对齐: 做多时 rs 正=好, 做空时 rs 负=好
        aligned = rs if sd == "bull" else -rs
        if aligned < -40:
            rb = "逆RS<-40"
        elif aligned < -15:
            rb = "逆RS-40~-15"
...
# ── 输出 ──
print()
print("=== 日线顺势入场 × 方向对齐RS (416天, 1:1) ===")
print("顺RS>40 = RS与方向一致最强 | 逆RS<-40 = RS与方向相反")
print(f"{'组合':<32} {'RS档':<14} {'n':>7} {'WR':>7} {'基差':>6}")
print("-" * 70)
...
for (sd, s4, s1), bd in sorted(base_wr.items(), key=lambda kv: -(kv[1]["w"]+kv[1]["l"])):
    ...
    for rb in ["逆RS<-40", "逆RS-40~-15", "RS中性", "顺RS15~40", "顺RS>40"]:
